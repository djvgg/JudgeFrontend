...
def seed():
    ...
    try:
        ...
        bracket_pool = (
            session.query(BracketModel).filter_by(group_id=group.id, bracket_type="POOL").first()
        )
        if not bracket_pool:
        ...
        for first, last in p_names:
            ...
            gp = (
                session.query(GroupParticipantModel)
                .filter_by(group_id=group.id, participant_id=p.id)
                .first()
            )
            if not gp:
        ...
        existing_fights_count = (
            session.query(FightModel).filter_by(bracket_id=bracket_pool.id).count()
        )
        ...
        group_de = (
            session.query(GroupModel).filter_by(age_group="U13", weight_class="-31kg").first()
        )
        if not group_de:
        ...
        bracket_de = (
            ...
            .filter_by(group_id=group_de.id, bracket_type="DOUBLE_ELIMINATION")
            .first()
        )
        if not bracket_de:
            bracket_de = BracketModel(
                group_id=group_de.id, bracket_type="DOUBLE_ELIMINATION", status="ongoing"
            )
        ...
        for first, last in de_p_names:
            ...
            gp = (
                session.query(GroupParticipantModel)
                .filter_by(group_id=group_de.id, participant_id=p.id)
                .first()
            )
            if not gp:
        ...
        if existing_de_fights == 0:
            ...
            session.add(
                FightModel(
                    bracket_id=bracket_de.id,
                    participant1_id=de_gps[0].id,
                    participant2_id=de_gps[1].id,
                    status="upcoming",
                    bracket_phase="wb",
                    round=1,
                    pos_in_round=0,
            ...
            session.add(
                FightModel(
                    bracket_id=bracket_de.id,
                    participant1_id=de_gps[2].id,
                    participant2_id=de_gps[3].id,
                    status="upcoming",
                    bracket_phase="wb",
                    round=1,
                    pos_in_round=1,
