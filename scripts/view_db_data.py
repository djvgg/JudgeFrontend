...
from src.database import MatchModel, SessionLocal
    ...
    try:
        with SessionLocal() as session:
            ...
            if total_matches > 0:
                ...
                first_matches = session.query(MatchModel).order_by(MatchModel.fight_nr).limit(3).all()

                for m in first_matches:
                    print(f"Combat n={m.fight_nr} (Table {m.table_id} - Round {m.round})")
